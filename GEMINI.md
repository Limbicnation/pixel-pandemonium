...
This project is a fast-paced, abstract lo-fi FPS developed using **Godot Engine 4.6**. It emphasizes momentum-based movement, visceral hitscan combat, and experimental "Trash Vision" mechanics.

## Project Overview

* **Core Concept:** A "trash game" aesthetic arena shooter where high-speed movement and aggressive combat are rewarded.
* **Engine:** Godot 4.6 (using the Forward+ renderer).
* **Main Technologies:** GDScript, CSG Primitives (for geometry), and specialized shaders for a neon-on-black aesthetic.
* **Architecture:**
  * **Autoloads:** `GameManager` handles global game state, scoring, and signal-based player registration.
...
* **Prerequisites:** Godot Engine 4.6.
...
Uses `direct_space_state.intersect_ray()` for instant hit detection. Damage is applied by calling `take_damage()` on the target's `HealthComponent`. Includes visual recoil, weapon bob, and optimized procedural effects (MeshInstance sparks and Tween-based muzzle flashes).

### Effects & Cleanup

Impact effects and muzzle flashes use `MeshInstance3D` and `OmniLight3D` managed by `Tween` for high-performance cleanup. Debris (shatter effects) use scene-based instancing with the `Debris` (debris.gd) component handling its own lifecycle and visual scaling.
